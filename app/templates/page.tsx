...
export default function TemplatesPage() {
    ...
    // File input keys to reset after upload
    ...
    // Track if current images are from saved gallery (already saved) vs newly uploaded
    ...
    // NEW: Team selection for milestone template
    ...
    // NEW: Selected playing XI players
    ...
    // Fetch saved images when team or template changes
    useEffect(() => {
        ...
        if (teamId) {
            // Pass template_type to filter images specific to this template
            fetchSavedImages(teamId, 'template', milestone || undefined, selectedTemplate);
        } else {
            setSavedImages([]);
        }
    }, [
    ...
    // Clear uploaded image only when template changes (not when logo selections change)
    useEffect(() => {
        setTossLayerImage('');
    ...
    // Fetch saved logos when team 1 is selected
    useEffect(() => {
        if (team1Selection) {
    ...
    // Fetch saved logos when team 2 is selected
    useEffect(() => {
        if (team2Selection) {
    ...
    // Fetch saved logos when milestone team is selected
    useEffect(() => {
        if (milestoneTeamId) {
    ...
    // Helper function to fetch saved logos for a team
    ...
    // Helper: Fetch saved images from database
    const fetchSavedImages = async (teamId?: number, imageType?: string, milestone?: string, templateType?: string) => {
        ...
        try {
            ...
            // If milestone provided, filter by it. If not, filter for NULL milestones (general images)
            if (milestone) {
            ...
            // Filter by template_type to ensure images are template-specific
            if (templateType) {
    ...
    // Helper: Save all images (template + logos) to database
    const saveAllImages = async () => {
        ...
        try {
            ...
            if (savedCount > 0 || duplicateCount > 0) {
                // Refresh saved template images
                if (teamId) fetchSavedImages(teamId, 'template', milestone || undefined, selectedTemplate);
                // Refresh saved logos
    ...
    // Helper: Get current template image
    ...
    // Helper: Clear current template image
    ...
    // Helper: Delete a saved image from database
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (imageType === 'template') {
                            ...
                            else if (selectedTemplate === 'milestone') { teamId = milestoneTeamId; milestone = milestoneForm.milestone.toString(); }
                            else if (selectedTemplate === 'fall_of_wicket') teamId = teams.find(t => t.name.toUpperCase() === fallOfWicketForm.battingTeam)?.team_id || null;
                            if (teamId) fetchSavedImages(teamId, 'template', milestone || undefined, selectedTemplate);
                        } else {
                            // Refresh logo lists
